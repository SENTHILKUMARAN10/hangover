...
const customer=read('ordering.js'),kitchen=read('cms-kitchen.js'),config=read('ordering-config.js');
...
assert.match(kitchen,/function validateOrder\(raw\)/,'Kitchen order validation missing');assert.match(kitchen,/raw\.table/,'Kitchen must validate table');assert.match(kitchen,/catalog\.get\(row\?\.id\)/,'Kitchen must calculate trusted catalog prices');assert.match(kitchen,/auth\.getUser\(\)/,'Kitchen must authenticate');assert.match(kitchen,/\['served'/,'Missing served status');
